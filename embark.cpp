...
void EmbarkExclusive::Run(color_ostream & out)
{
    ...
    While(viewscreen_is_not<df::viewscreen_dwarfmodest>, [&]()
    {
        ...
        If(viewscreen_is<df::viewscreen_adopt_regionst>, [&]() { ViewWait(); });
        If(viewscreen_is<df::viewscreen_export_regionst>, [&]() { ViewWait(); });
        If(viewscreen_is<df::viewscreen_game_cleanerst>, [&]() { ViewWait(); });
        If(viewscreen_is<df::viewscreen_movieplayerst>, [&]() { ViewMoviePlayer(); });
        If(viewscreen_is<df::viewscreen_titlest>, [&]() { ViewTitle(out); });
        If(viewscreen_is<df::viewscreen_loadgamest>, [&]() { ViewLoadGame(out); });
        If(viewscreen_is("dfhack/lua/load_screen"), [&]() { ViewLoadScreen(out); });
        If([&]() -> bool
        {
        ...
        If(viewscreen_is<df::viewscreen_setupdwarfgamest>, [&]() { ViewSetupDwarfGame(out); });
        If(viewscreen_is<df::viewscreen_textviewerst>, [&]() { ViewTextViewer(out); });
