...
{
    auto & k = ui->kitchen;
    std::map<std::tuple<int16_t, int32_t, df::item_type, int16_t>, int8_t> already_banned;
    for (size_t i = 0; i < k.item_types.size(); i++)
    {
        already_banned[std::make_tuple(k.mat_types[i], k.mat_indices[i], k.item_types[i], k.item_subtypes[i])] = k.exc_types[i];
    }
    auto ban_cooking = [this, &out, &already_banned](int16_t mat_type, int32_t mat_index, df::item_type type)
    {
        int16_t subtype = -1;
        ...
        ai->debug(out, "ban_cooking " + ENUM_KEY_STR(item_type, type) + ":" + mat.getToken());
        auto & k = ui->kitchen;
        if (exists)
        {
            for (size_t i = 0; i < k.item_types.size(); i++)
            {
                if (k.mat_types[i] == mat_type &&
                        k.mat_indices[i] == mat_index &&
                        k.item_types[i] == type &&
                        k.item_subtypes[i] == subtype)
                {
                    k.exc_types[i] |= 1;
                    break;
                }
        ...
        {
            k.mat_types.push_back(mat_type);
            k.mat_indices.push_back(mat_index);
            k.item_types.push_back(type);
            k.item_subtypes.push_back(subtype);
            k.exc_types.push_back(1); // cooking
        }
    };
    ...
    for (size_t i = 0; i < world->raws.plants.all.size(); i++)
    {
        ...
        for (size_t j = 0; j < p->material.size(); j++)
        {
            ...
            if (m->flags.is_set(material_flags::ALCOHOL))
            {
                ban_cooking(j + MaterialInfo::PLANT_BASE, i, item_type::DRINK);
            }
        }
    }
    for (size_t i = 0; i < world->raws.creatures.all.size(); i++)
    {
        ...
        for (size_t j = 0; j < c->material.size(); j++)
        {
            ...
            if (m->flags.is_set(material_flags::ALCOHOL))
            {
                ban_cooking(j + MaterialInfo::CREATURE_BASE, i, item_type::DRINK);
            }
        }
    }
    ...
    for (size_t i = 0; i < world->raws.creatures.all.size(); i++)
    {
        ...
        {
            df::material *m = c->material[j];
            if (has_reaction_product(m, "SOAP_MAT"))
            {
                ban_cooking(j + MaterialInfo::CREATURE_BASE, i, item_type::GLOB);
            }
        }
    }
    ...
    for (size_t i = 0; i < world->raws.plants.all.size(); i++)
    {
        ...
        if (has_reaction_product(basic.material, "SEED_MAT"))
        {
            ban_cooking(basic.type, basic.index, item_type::PLANT);
        }

        if (!p->flags.is_set(plant_raw_flags::TREE))
        {
            for (df::plant_growth *g : p->growths)
            {
                MaterialInfo m(g);
                if (has_reaction_product(m.material, "SEED_MAT"))
                {
                    ban_cooking(m.type, m.index, item_type::PLANT_GROWTH);
                }
            }
        }
    }
